...
export default class LuxgateCoinInfoStore extends Store {
  ...
  // REQUESTS
  @observable
  getCoinInfoRequest: Request<GetCoinInfoResponse> = new Request(this.api.luxgate.getCoinInfo);
  @observable
  ...
  _sendCoin = async (transactionDetails: { coin: string, receiver: string, amount: number }) => {
  ...
  */
  @action
  getCoinInfoData = async (coin: string) => {
    const isLogined = this.stores.luxgate.loginInfo.isLogined;
    if (!isLogined) return;
    ...
    const coinInfo: GetCoinInfoResponse = await this.getCoinInfoRequest.execute(password, coin)
      .promise;
    if (coinInfo != null) {
      if (coin == coinInfo.coin) {
        const address = coinInfo.smartaddress;
        const balance = coinInfo.balance
          ? coinInfo.balance
          : await this.getCoinBalanceRequest.execute(password, coin, address).promise;
  ...
  @computed
  get coinInfoList(): Array<CoinInfo> {
    return this.lstCoinInfo;
  }

  @action
  _addCoinInfo = (info: CoinInfo) => {
    for (let i = 0; i < this.lstCoinInfo.length; i++) {

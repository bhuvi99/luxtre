...
export default class LuxgateLoginInfoStore extends Store {
  // REQUESTS
  @observable
  LuxgateLoginRequest: Request<GetCoinInfoResponse> = new Request(this.api.luxgate.getCoinInfo);
  @observable
  getAccountNewPhraseRequest: Request<GetAccountNewPhraseResponse> = new Request(
    this.api.luxgate.getAccountNewPhrase
  );
  @observable
  getPasswordInfoRequest: Request<GetPasswordInfoResponse> = new Request(
    this.api.luxgate.getPasswordInfo
  );
  ...
  @observable
  password: string = LUXGATE_PASSWORD;
  @observable
  myPhrase: string = LUXGATE_USER;
  ...
  _createNewPhrase = async () => {
    try {
      const newPhrase: ?GetAccountNewPhraseResponse = await this.getAccountNewPhraseRequest.execute()
        .promise;
      if (newPhrase != null) {
  ...
  @action
  _mapNewPhrase = (newPhrase: GetAccountNewPhraseResponse) => {
    this.newPhraseWords = newPhrase.map(word => ({ word }));
  };

  @action
  _loginWithPhrase = async (phrase: string) => {
    const password = LUXGATE_PASSWORD;
    this.myPhrase = phrase;
    const info: GetPasswordInfoResponse = await this.getPasswordInfoRequest.execute(
      password,
      phrase
    ).promise;
    if (info !== '') {

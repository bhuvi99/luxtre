...
export const messages = defineMessages({
  ...
  backupInstructions: {
    ...
    description:
      'Instructions for backing up recovery phrase on dialog that displays recovery phrase.'
  },
  buttonLabelLogin: {
...
export default class LuxgateLoginDialog extends Component<Props, State> {
  ...
  changeAccountInput(value) {
    ...
    if (value1 == value2) this.setState({ isMatched: true });
  ...
  render() {
    ...
    const { newPhrase, onCopyAddress, error, onCancel, children } = this.props;

    const { account, isMatched, isNewPhrase } = this.state;

    const inputStyle = classnames([isNewPhrase ? styles.zeroMargin : styles.password]);
